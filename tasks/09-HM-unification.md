...
- Сигнатуры стали опциональны,
- Вместо типа теперь можно подставить символ `_` (_type-hole_) 
  — указание компилятору вывести тип самостоятельно.

```diff
*** grammar-v4.txt	2023-02-25 12:35:13.528570009 +0300
--- grammar-v5.txt	2023-03-02 10:51:38.750028108 +0300
***************
*** 12,14 ****
...
На алгоритме юнификации построен язык и система логического программирования **Prolog**.

- Сейчас мы способны выразить логику **исчисления высказываний**,
- Как только мы научимся выражать **полиморфные функции**, то наш язык высказываний пополнится
  квантором всеобщности $\forall$ — т.е. язык станет способен выразить _логику первого порядка_,  
- Как только мы научимся выражать **классы типов**, то наш язык высказываний пополнится 
  определяемыми пользователем предикатами на типах.

## В следующих сериях

Мы узнаем, как можно представить себе _полиморфную функцию_ `id` и как обращаться с ней
